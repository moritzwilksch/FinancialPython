...
def create_comparison(tickers):
    ...
    for ticker in tickers:
        try:
            ...
            print(f"[INFO] Did not find {ticker} on disk. Fetching from API...")
...
def load_data(tickers):
    return create_comparison(tickers)



st.title('Stock Peer Group Analysis')
to_load = st.text_area('Choose tickers to load into application (comma-separated)').split(',')

df = load_data(to_load)


tickers = st.multiselect(label="Stock Tickers", options=to_load, default=to_load)

st.markdown("---")

st.markdown(df[tickers].to_markdown())
