...
def simulate(cutoff, n_samples=2_500, thresh: Literal['lower', 'upper', 'both']='both'):
    ...
    return (
        lower < yft['Close'].iloc[cutoff + DTE] < upper if thresh=='both' else
        yft['Close'].iloc[cutoff + DTE] < upper if thresh=='upper' else
        yft['Close'].iloc[cutoff + DTE] > lower
        )


result = Parallel(n_jobs=-1)(delayed(simulate)(cutoff, thresh=THRESH) for cutoff in range(start, len(returns) - DTE))

fig, ax = plt.subplots(figsize=(15, 6))
ax.plot(list(range(start, len(returns) - DTE)), (np.array(result).cumsum()/range(1, len(result) + 1)), c='blue', zorder=20)
